...
class CompraAdmin(ModelAdmin):
    list_display = ('usuario', 'status', 'total_formatado', 'data_criacao')
    search_fields = ('usuario', 'status', 'data_criacao')
    list_filter = ('usuario', 'status')
    ordering = ('usuario', 'status')
    list_per_page = 10
    inlines = [ItensCompraInline]
    readonly_fields = ('data_criacao', 'total_formatado')

    @display(description='Total')
    def total_formatado(self, obj):
        """Exibe R$123,45 ao invés de 123,45"""
